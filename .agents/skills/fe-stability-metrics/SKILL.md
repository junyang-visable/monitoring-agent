...
4. 写入以项目名单隔离的 `{OUTPUT_DIR}/analysis-{scope_key}-{comparison_id}.json`
...
路径：`{OUTPUT_DIR}/analysis-{scope_key}-{comparison_id}.json`。`scope_key` 为项目按字典序拼接后 SHA-256 前 12 位的 `apps-<hash>`。
