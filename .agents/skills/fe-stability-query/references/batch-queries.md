...
本文件是唯一 SQL 模板。`{APP_LIST}` 必须由编排器已解析、去重的标准 `app_name` 安全替换为 SQL 字符串列表。所有查询覆盖 CURR/BASE，返回 `app_name` 和 `period`，并只过滤选中的项目。

## A：每日总量

```sql
SELECT app_name,
  CASE WHEN ds BETWEEN '{CURR_START}' AND '{CURR_END}' THEN 'CURR' ELSE 'BASE' END AS period,
  ds, COUNT(*) AS cnt
...
GROUP BY app_name, period, ds
ORDER BY app_name, period, ds
```
...
```sql
WITH grouped AS (
  SELECT app_name, CASE WHEN ds BETWEEN '{CURR_START}' AND '{CURR_END}' THEN 'CURR' ELSE 'BASE' END AS period,
