...
1. 读取由上游显式传入的 `analysis-{scope_key}-{comparison_id}.json`（schema ≥ 1.3）
...
若 `meta.schema_version` < `"1.3"` 或缺少 `priority_actions`，提示需重新运行 `fe-stability-metrics`。
...
| 范围 | 模板 | 输出 |
|---|---|---|
| `app_names` | [references/template-by-apps.md](references/template-by-apps.md) | `frontend-stability-apps-{scope_key}-{comparison_id}.md`；按 `meta.app_names` 渲染每个项目章节 |
...
草稿：`{OUTPUT_DIR}/.draft-frontend-stability-apps-{scope_key}-{comparison_id}.md`
