...
9. Write one Markdown, one JSON, and one standalone HTML report for the run. The HTML report is generated by invoking `report_html.generate_html_report(run_dir)` and writing the output to `artifacts/monitoring/<run_id>/report.html`.
...
- Reports → `artifacts/monitoring/<run_id>/report.md`, `report.json`, and `report.html`
- HTML report generator → `monitoring-orchestrator/report_html.py`

Step 9 (write reports) MUST follow [references/report-format.md](references/report-format.md). The report is a signal-level dashboard; detailed analysis stays in evidence JSONs. The HTML report is part of Step 9 — after writing `report.md` and `report.json`, invoke `report_html.generate_html_report(run_dir)` and write the result to `<run_dir>/report.html`.
