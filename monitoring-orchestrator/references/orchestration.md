...
| Stability SDK | `fe-stability-analysis` (`app_names`, `output_mode=analysis_only`) | one batch query plan for enabled projects; distribute normalized `by_app` summaries for the equivalent calendar-day range; no separate Stability Markdown report |
