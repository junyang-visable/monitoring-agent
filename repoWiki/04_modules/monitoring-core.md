...
- Key Files → `monitoring-orchestrator/adapters/{datadog,sentry,github}.py` → platform HTTP adapters; `monitoring-orchestrator/adapters/common.py` → shared runtime helpers; `monitoring-orchestrator/runtime.py` → backward-compatible facade; `monitoring-orchestrator/references/contracts.md` → result contract.
