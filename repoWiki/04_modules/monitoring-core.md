...
- Constraints → parallel paths; independent timeouts; normalized results; no fabricated values; Datadog service override falls back to `app_name`; Datadog, Stability SDK, and Sentry inherit a shared `time_range`; Stability SDK batches enabled projects by `app_name`.
- Symbols → none yet.
